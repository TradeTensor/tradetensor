...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            if (fCheckZTNSR && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zTNSR in the future, for now it's useless,
...
bool CWallet::CreateZTNSROutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZTNSR(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, std::vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZTNSROutPut(denomination, outMint, dMint)) {
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    for (auto &it : mapMintsSelected) {
        ...
        CTxIn in;
        if(!ZTNSRModule::createInput(in, mint, hashTxOut, spendVersion)) {
        ...
        bool isPublicSpend)
{
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zTNSR"), nStatus);
        return false;
    }
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one zTNSR public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zTNSR used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZTNSR_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZTNSR_SPEND_OKAY); // Everything okay
...
void CWallet::setZWallet(CzTNSRWallet* zwallet)
{
    zwalletMain = zwallet;
    zpivTracker = std::unique_ptr<CzTNSRTracker>(new CzTNSRTracker(strWalletFile));
}

CzTNSRWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
