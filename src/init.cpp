...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 27413, 37413));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, tradetensor, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-backupztnsr=<n>", strprintf(_("Enable automatic wallet backups triggered after each zTnsr minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizetradetensoramount=<n>     " + strprintf(_("Keep N TNSR anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 37413, 38843));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Tradetensor Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZTnsrBackups = GetBoolArg("-backupztnsr", true);
        pwalletMain->setZTnsrAutoBackups(fEnableZTnsrBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeTradetensorAmount = GetArg("-anonymizetradetensoramount", 0);
//    if (nAnonymizeTradetensorAmount > 999999) nAnonymizeTradetensorAmount = 999999;
//    if (nAnonymizeTradetensorAmount < 2) nAnonymizeTradetensorAmount = 2;
    ...
    LogPrintf("Anonymize Tradetensor Amount %d\n", nAnonymizeTradetensorAmount);
