        ...
        if (pblock->nVersion < 7) {
            if (fZerocoinActive) {
                ...
                if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
                    //For the period before v2 activation, zTNSR will be disabled and previous block's checkpoint is all that will be needed
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zTNSR with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    return NULL;
                }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock !=  chainActive.Tip()->GetBlockHash())
            return error("TradeTensorMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("TradeTensorMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("TradeTensorMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tradetensor-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running TradeTensorMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("TradeTensorMiner exception");
    } catch (...) {
        LogPrintf("TradeTensorMiner exception");
    }

    LogPrintf("TradeTensorMiner exiting\n");
}
