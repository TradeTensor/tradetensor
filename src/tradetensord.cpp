...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/tradetensor.conf are parsed in qt/tradetensor.cpp's main()
    ...
    try {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "tradetensor:"))
                fCommandLine = true;

        if (fCommandLine) {
            fprintf(stderr, "Error: There is no RPC client functionality in tradetensord anymore. Use the tradetensor-cli utility instead.\n");
            exit(1);
        }
#ifndef WIN32
...
int main(int argc, char* argv[])
{
    ...
    // Connect tradetensord signal handlers
    noui_connect();
