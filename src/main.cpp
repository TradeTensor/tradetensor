...
#if defined(NDEBUG)
#error "TradeTensor cannot be compiled without assertions."
#endif
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
        return error("%s : zTNSR spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zTNSR is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 zTNSR spend does not have a valid signature\n", __func__);
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend zTNSR without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend->HasValidSerial(Params().Zerocoin_Params(fUseV1Params)))  {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zTNSR spend with serial %s from tx %s is not in valid range\n", __func__,
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZTNSRModule::validateInput(txin, prevOut, tx, ret)){
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zTNSR spend tx %s already in block %d",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 600 * COIN;
    } else if (nHeight > 500 && nHeight <= 7000) {
        nSubsidy = 1 * COIN;
    } else if (nHeight > 7000 && nHeight <= 20000) {
        nSubsidy = 1.5 * COIN;
        ...
        nSubsidy = 2 * COIN;
    } else if (nHeight > 250000 && nHeight <= 4666120) {
        nSubsidy = 1 * COIN;
    } else {
        nSubsidy = 7.5 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from TradeTensor
...
{
    RenameThread("tradetensor-scriptch");
...
bool UpdateZTNSRSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zTNSR supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateTNSRSupply(Params().Zerocoin_StartHeight());
    }

    //Track zTNSR money supply in the block index
    if (!UpdateZTNSRSupply(block, pindex, fJustCheck))
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        ...
        // cold staking is active much after nPublicZCSpends so GetMasternodePayment is always 3 TNSR.
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // TradeTensor Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t) Params().TradeTensorBadBlockTime()) && (block.nBits == (uint32_t) Params().TradeTensorBadBlocknBits())) {
            // accept TradeTensor block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        const bool hasTNSRInputs = !pivInputs.empty();
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if(hasZTNSRInputs && in.IsZerocoinSpend())
            ...
            if(hasZTNSRInputs) {
                for (const CTxIn& zPivInput : zTNSRInputs) {
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // TradeTensor: We use certain sporks during IBD, so check to see if they are
