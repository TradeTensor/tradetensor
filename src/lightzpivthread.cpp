...
void CLightWorker::ThreadLightZTNSRSimplified() {
    RenameThread("tradetensor-light-thread");
    ...
    while (true) {
        try {
            ...
            LogPrintf("%s pop work for %s \n\n", "tradetensor-light-thread", genWit.toString());
            ...
            if (!pIndex) {
                ...
                if (blockHeight >= Params().Zerocoin_Block_V2_Start()) {
                    ...
                    if (!res) {
                        ...
                        ss << heightStop;
                        if (genWit.getPfrom()) {
                            LogPrintf("%s pushing message to %s \n", "tradetensor-light-thread", genWit.getPfrom()->addrName);
...
void CLightWorker::rejectWork(CGenWit& wit, int blockHeight, uint32_t errorNumber) {
    if (wit.getStartingHeight() == blockHeight){
        LogPrintf("%s rejecting work %s , error code: %s\n", "tradetensor-light-thread", wit.toString(), errorNumber);
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
