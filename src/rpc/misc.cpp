...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total tradetensor balance of the wallet\n" // (excluding zerocoins)\n"
...
*/
            ...
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in tradetensor/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in tradetensor/kb\n"
...
/*
    ...
    obj.push_back(Pair("zTNSRsupply", zpivObj));
*/
#ifdef ENABLE_WALLET
    if (pwalletMain) {
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"tradetensoraddress\"\n"
            "\nReturn information about the given tradetensor address.\n"

            "\nArguments:\n"
            "1. \"tradetensoraddress\"     (string, required) The tradetensor address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"tradetensoraddress\",    (string) The tradetensor address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for TradeTensor cold staking\n"
...
#ifdef ENABLE_WALLET
        // Case 1: TradeTensor address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are tradetensor addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) tradetensor address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"tradetensoraddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"tradetensoraddress\"  (string, required) The tradetensor address to use for the signature.\n"
...
UniValue getstakingstatus(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"staking_enabled\": true|false,    (boolean) if staking is enabled/disabled in tradetensor.conf\n"
