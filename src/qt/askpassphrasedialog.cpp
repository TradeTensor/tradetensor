...
#include "qt/tradetensor/loadingdialog.h"
    ...
    case Mode::Encrypt: {
        ...
        if (ret) {
            if (newpass1 == newpass2) {
                ...
                TradeTensorGUI* window = static_cast<TradeTensorGUI*>(parentWidget());
...
{
    TradeTensorGUI* gui = static_cast<TradeTensorGUI*>(parentWidget());
...
{
    hide();
    static_cast<TradeTensorGUI*>(parentWidget())->showHide(true);
    openStandardDialog(
            tr("Wallet encrypted"),
            "<qt>" +
            tr("TradeTensor will close now to finish the encryption process. "
