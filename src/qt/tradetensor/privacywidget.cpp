...
#include "qt/tradetensor/privacywidget.h"
#include "qt/tradetensor/forms/ui_privacywidget.h"
#include "qt/tradetensor/qtutils.h"
#include "guiutil.h"
#include "qt/tradetensor/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(TradeTensorGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zTNSR anonymizes your TNSR by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zTNSR or convert back to TNSR"));
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zTNSR");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zTNSR");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zTNSR");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zTNSR");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zTNSR");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zTNSR");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zTNSR");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zTNSR");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zTNSR Movements"));
    ...
    // Only Convert to TNSR enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzTNSR->setTitleClassAndText("btn-title-grey", "Total 0 zTNSR");
    ui->btnTotalzTNSR->setSubTitleClassAndText("text-subtitle", "Show denominations of zTNSR owned.");
    ui->btnTotalzTNSR->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select TNSR outputs to mint into zTNSR.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zTNSR");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzTNSR, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zTNSR");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of TNSR to mint into zTNSR"));
    }else{
        btnText = tr("Convert back to TNSR");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zTNSR to convert back into TNSR"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzTNSR->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzTNSR->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zTNSR is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zTNSR, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zTNSR").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zTNSR minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zTNSR converted back to TNSR"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzTNSR->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
