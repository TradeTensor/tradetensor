...
#include "qt/tradetensor/send.h"
#include "qt/tradetensor/forms/ui_send.h"
#include "qt/tradetensor/addnewcontactdialog.h"
#include "qt/tradetensor/qtutils.h"
#include "qt/tradetensor/sendchangeaddressdialog.h"
#include "qt/tradetensor/optionbutton.h"
#include "qt/tradetensor/sendconfirmdialog.h"
#include "qt/tradetensor/myaddressrow.h"
#include "qt/tradetensor/guitransactionsutils.h"
...
SendWidget::SendWidget(TradeTensorGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("TradeTensor address or contact label"));
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_TNSR : AskPassphraseDialog::Context::Send_zTNSR, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zTNSR is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zTNSR transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZTNSR_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zTNSR require a security level of 100 to successfully spend.");
        } else {
        ...
        emit message("zTNSR transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isTNSR){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zTNSR in your balance to select."));
        }
    }
}
...
void SendWidget::onTNSRSelected(bool _isTNSR){
    isTNSR = _isTNSR;
    setCssProperty(coinIcon, _isTNSR ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
