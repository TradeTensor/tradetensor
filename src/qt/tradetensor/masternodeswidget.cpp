...
#include "qt/tradetensor/forms/ui_masternodeswidget.h"
#include "qt/tradetensor/qtutils.h"
#include "qt/tradetensor/mnrow.h"
#include "qt/tradetensor/mninfodialog.h"

#include "qt/tradetensor/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(TradeTensorGUI *parent) :
    PWidget(parent),
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= CollateralRequired(chainActive.Height())){
            ...
            // kind of like "...node, %i TNSR requited").arg(qYourCollateralResultVariable),
            // to nicely print required collateral amount.
            inform(tr("Not enough balance to create a masternode")); //, 10,000 TNSR required."));
            return;
        }
        showHideOp(true);
