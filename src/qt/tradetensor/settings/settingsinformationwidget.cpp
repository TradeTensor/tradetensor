...
#include "qt/tradetensor/settings/forms/ui_settingsinformationwidget.h"
...
SettingsInformationWidget::SettingsInformationWidget(TradeTensorGUI* _window,QWidget *parent) :
    ...
    ui(new Ui::SettingsInformationWidget)
{
    ...
    connect(ui->pushButtonFile, &QPushButton::clicked, [this](){
        if (!GUIUtil::openConfigfile())
            inform(tr("Unable to open tradetensor.conf with default application"));
    });
