...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (tradetensor.conf).");
            QMessageBox::warning(this, "TradeTensor Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
