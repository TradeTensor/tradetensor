...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Tradetensor address (e.g. %1)").arg("PCYiHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
{
    // return if URI is not valid or is no Tradetensor: URI
...
{
    // Convert tradetensor:// to tradetensor:
    //
    //    Cannot handle this later, because tradetensor:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open tradetensor.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Tradetensor.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Tradetensor.lnk
...
{
    return GetAutostartDir() / "tradetensor.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a tradetensor.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Tradetensor\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the tradetensor app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add tradetensor app to startup item list
