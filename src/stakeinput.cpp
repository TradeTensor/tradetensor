...
bool CZPivStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zTNSR staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
bool CZPivStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    ...
    if (!pwallet->CreateZTNSROutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zTNSR output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zTNSR", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZTNSROutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zTNSR output", __func__);
...
{
    CzTNSRTracker* zpivTracker = pwallet->zpivTracker.get();
    CMintMeta meta;
