...
- [Development guidelines](#development-guidelines)
    - [General TradeTensor Core](#general-tradetensor-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
TradeTensor Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
TradeTensor Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/tradetensor-project/tradetensor/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_tradetensor
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_tradetensor --log_level=test_suite
$ valgrind -v --leak-check=full src/tradetensord -printtoconsole
```
...
# A coverage report will now be accessible at `./test_tradetensor.coverage/index.html`.
```
...
However, in open source software such as TradeTensor Core, where everyone uses
...
pay attention to for reviewers of TradeTensor Core code.

General TradeTensor Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: TradeTensor Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against TradeTensor Core so that they can be integrated
...
file descriptors it will cause problems with TradeTensor's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:TradeTensor-Project/TradeTensor.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `tradetensor-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `tradetensor-cli` and the GUI debug console use this table to determine how to
