...
*Setup instructions for a gitian build of Tradetensor using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Tradetensor
...
- [Building Tradetensor](#building-tradetensor)
...
Clone the git repositories for tradetensor and gitian and then checkout the tradetensor version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the tradetensor repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building Tradetensor
----------------

To build Tradetensor (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the tradetensor repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/tradetensor/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/tradetensor-project/tradetensor.git
...
For example:
```bash
URL=https://github.com/tradetensor-project/tradetensor.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit tradetensor=${COMMIT} --url tradetensor=${URL} ../Tradetensor/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit tradetensor=${COMMIT} --url tradetensor=${URL} ../Tradetensor/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit tradetensor=${COMMIT} --url tradetensor=${URL} ../Tradetensor/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tradetensor-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tradetensor-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/tradetensor-build.assert
```
...
[tradetensor-project/tradetensor/gitian.sigs](https://github.com/tradetensor-project/tradetensor/gitian.sigs/) repository, or if that's not possible to create a pull
request.
