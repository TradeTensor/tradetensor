...
*Setup instructions for a gitian build of TradeTensor Core using a VM or physical system.*
...
to the TradeTensor GitHub release page.
...
- [Building TradeTensor Core](#building-tradetensor-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/tradetensor-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/tradetensor-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/tradetensor-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/tradetensor-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/tradetensor-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/tradetensor-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/tradetensor-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@tradetensor.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/tradetensor-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/tradetensor-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/tradetensor-osx-*-build.assert
```
