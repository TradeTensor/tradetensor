TradeTensor Core version *v4.0.0* is now available from:  <https://github.com/tradetensor-project/tradetensor/releases>
...
TradeTensor Core v4.0.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
TradeTensor Core is extensively tested on multiple operating systems using the Linux kernel, macOS 10.10+, and Windows 7 and later.

Microsoft ended support for Windows XP on [April 8th, 2014](https://www.microsoft.com/en-us/WindowsForBusiness/end-of-xp-support), No attempt is made to prevent installing or running the software on Windows XP, you can still do so at your own risk but be aware that there are known instabilities and issues. Please do not report issues about Windows XP to the issue tracker.

Apple released it's last Mountain Lion update August 13, 2015, and officially ended support on [December 14, 2015](http://news.fnal.gov/2015/10/mac-os-x-mountain-lion-10-8-end-of-life-december-14/). TradeTensor Core software starting with v3.2.0 will no longer run on MacOS versions prior to Yosemite (10.10). Please do not report issues about MacOS versions prior to Yosemite to the issue tracker.

TradeTensor Core should also work on most other Unix-like systems but is not frequently tested on them.
...
You can read more details about this extensive work in ([PR #954](https://github.com/TradeTensor-Project/TradeTensor/pull/954))
...
A brand new feature is being introduced with the release of v4.0.0: Cold Staking ([PR #955](https://github.com/TradeTensor-Project/TradeTensor/pull/955))! This feature allows a coin owner to keep coins in a "cold" (or locked) wallet whilst a "hot" wallet carries out the burden of staking those coins.
...
A full technical writup is available on the [TradeTensor Wiki](https://github.com/TradeTensor-Project/TradeTensor/wiki/ColdStaking), and an initial video showcase is available on [YouTube](https://www.youtube.com/watch?v=utxB5TzAeXc).
A brief guide to setup cold staking with GUI and RPC is available [here](https://github.com/random-zebra/TradeTensor-Wiki/blob/master/User-Documentation/Cold-Staking-HowTo.md).

<sup>1</sup> Spending cold stakes from HW wallets currently available only for Ledger devices via [PET4L](https://github.com/TradeTensor-Project/PET4L) tool.

Multi-Split Stake Splitting
-------------------

Stake splitting has received a makeover and now supports splitting to more than two (2) outputs. [PR #968](https://github.com/TradeTensor-Project/TradeTensor/pull/968) introduced the change, which is controlled by the wallet's `stakesplitthreshold` setting.

The default split threshold remains at 2000 TNSR, and can be adjusted in the GUI's Settings page, or via the RPC `setstakesplitthreshold` command.
...
### V1 zTNSR Spending (Public Spends Version 4)

Since the discovery of a critical exploit within the libzerocoin library in early 2019, remaining legacy v1 zTNSR have been un-spendable. We're happy to say that, once the new consensus rules are in effect, users will once again be able to spend their v1 zTNSR with public spends version 4 ([PR #936](https://github.com/TradeTensor-Project/TradeTensor/pull/936)).

As with the previous version 3 public spends introduced in core wallet version 3.3.0 (enabling the spending of v2 zTNSR), version 4 spends will also be public. A full technical writeup is available on the [TradeTensor Wiki](https://github.com/TradeTensor-Project/TradeTensor/wiki/CoinRandomnessSchnorrSignature).
...
[#PR1022](https://github.com/TradeTensor-Project/TradeTensor/pull/1022) defines Version 7 blocks, which remove the (now-unused) accumulator checkpoint from the block header. This results in an overall data reduction of ~256 bits from each block as well as the in-memory indexes.

### New Network Message Signatures

Layer 2 network messages (MN, Budget, Spork, etc) are now signed based on the hash of their **binary** content instead of their **string** representation ([#PR1024](https://github.com/TradeTensor-Project/TradeTensor/pull/1024)).

### New SPORKS

Two new SPORKS are introduced, `SPORK_17` ([#PR975](https://github.com/TradeTensor-Project/TradeTensor/pull/975)) and `SPORK_18` ([#PR995](https://github.com/TradeTensor-Project/TradeTensor/pull/995)).<br>
...
- `delegated_balance` - TNSR balance held in P2CS contracts (delegated amount total).
- `cold_staking_balance` - TNSR balance held in cold staking addresses.
...
For our linux users, in addition to the [Ubuntu PPA](https://launchpad.net/~tradetensor) repository, we are now offering a [Snap package](https://snapcraft.io/tradetensor-core) as quick way to install and update a TradeTensor wallet.
...
- #936 `12a6b704b6` [zTNSR] PublicCoinSpend v4 - Coin Randomness Schnorr Signature (random-zebra)
...
- #1129 `a87bfc32a0` [Consensus] Define TestNet changeover block for TradeTensor v4.0 RC (random-zebra)
- #1134 `a4ded20de4` [Trivial] Remove a duplicate variable definition (warrows)
- #1191 `9a054eeba6` [Consensus] Define MainNet changeover block for TradeTensor v4.0 (random-zebra)
...
- #1125 `c6e238ca4d` [GUI] Inform if open tradetensor.conf and/or backups folder fail. (furszy)
...
- #1179 `7a33fe10d2` [Qt] Fix for dead link to wrong TradeTensor website (NoobieDev12)
