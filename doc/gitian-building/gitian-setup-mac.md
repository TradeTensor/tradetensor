...
The build script we'll be using is contained in the TradeTensor github repository ([contrib/gitian-build.py](https://github.com/tradetensor-project/tradetensor/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the TradeTensor repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/TradeTensor-Project/TradeTensor/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, tradetensor-detached-sigs, and tradetensor GitHub repos.
