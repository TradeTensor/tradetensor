...
====================================
This guide will show you how to build tradetensord (headless client) for OSX.
...
4.  (Optional) You can also install tradetensord to your path:
...
Set the **Build directory** path to the `root` of the clone Tradetensor project. Qt Creator needs this to locate the `Makefile`.
...
To start debugging, locate the following function in `tradetensor.cpp` and set a breakpoint.
...
------------------------
You can ignore this section if you are building `tradetensord` for your own use.

`tradetensord/tradetensor-cli` binaries are not included in the `tradetensor-Qt.app` bundle.

If you are building `tradetensord` or `tradetensor-qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the Tradetensor-Qt.app
bundle is packaged and signed to create the .dmg disk image that is distributed.

Running
-------

It's now available at `./tradetensord`, provided that you are still in the `src`
directory. We have to first create the RPC configuration file, though.

Run `./tradetensord` to get the filename where it should be put, or just try these
commands:

    echo -e "rpcuser=tradetensorrpc\nrpcpassword=$(xxd -l 16 -p /dev/urandom)" > "/Users/${USER}/Library/Application Support/Tradetensor/tradetensor.conf"
    chmod 600 "/Users/${USER}/Library/Application Support/Tradetensor/tradetensor.conf"

The next time you run it, it will start downloading the blockchain, but it won't
output anything while it's doing this. This process may take several hours;
you can monitor its process by looking at the debug.log file, like this:

    tail -f $HOME/Library/Application\ Support/Tradetensor/debug.log

Other commands:
-------

    ./tradetensord -daemon # to start the tradetensor daemon.
    ./tradetensor-cli --help  # for a list of command-line options.
    ./tradetensor-cli help    # When the daemon is running, to get a list of RPC commands
